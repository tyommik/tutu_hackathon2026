...
/**
 * Пул кандидатов: LLM-хабы идут первыми, но статический список всегда
 * в хвосте страховкой. Замещение (а не слияние) уже стреляло: для
 * Воронеж → Каир модель выдала пять международных хабов, Москва выпала
 * из перебора — а единственный собираемый маршрут шёл именно через неё
 * (из регионального города за рубеж первым звеном обычно домашний хаб).
 */
export function hubPool(suggested?: string[] | null): string[] {
  return suggested ? [...suggested, ...HUBS] : HUBS;
}
 ...
 * см. hubSuggest.ts и hubPool), дубли в нём срезаем.
 */
